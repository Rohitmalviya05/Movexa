...
exports.myDriverBooking = async (req, res) => {
  try {
    ...
    if (!booking) return res.json({ message: "No current booking" });
...
exports.getAvailableBooking = async (req, res) => {
  try {
    ...
    const booking = await Booking.findOne({ status: "CREATED" }).sort({ createdAt: 1 });

    if (!booking) {
      return res.json({ message: "No bookings available" });
    }
